...
void ProcessRun(int RunID, int ID)
{
  ...
  TH1F *hN = new TH1F("hN","Charge Energy Spectrum (purity - 2us)",200,0,1000);
  TH1F *hP = new TH1F("hP","Charge Energy Spectrum (purity + 2us)",200,0,1000);
  TH1F *hMulti = new TH1F("hMulti","Multi Site Charge Energy Spectrum (mean purity)",200,0,1000);

  char FileName[100];
  sprintf(FileName,"/EXO200Data/Disk3/processed/%i/recon0000%i-*.root",RunID,RunID);
  ...
  TF1 *fit = new TF1("fit",fitFunction,450,800,4);
  fit->SetParNames("A1","E1","#sigma","A2");

  fit->SetParameter(1,630);
  ...
  TF1 *fitMulti = new TF1("fitMulti","gaus",540,680);
