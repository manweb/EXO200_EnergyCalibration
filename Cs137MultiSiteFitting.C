...
void Cs137MultiSiteFitting()
{
  ...
  for (int i = 0; i < nrRuns; i++) {
     char fname[100];
     sprintf(fname,"../analysis/Tree/%i_noReclustering_Fiducial_thresh.root",runID[i]);
  ...
  TH1F *h = new TH1F("h","multi site spectrum",200,0,1000);

  t->Draw("ecrec>>h","nsite > 1");

  TF1 *fit = new TF1("fit","gaus",530,650);
  //fit->SetParNames("A1","E1","#sigma","R");
  //fit->SetParameters(8000,1150,100,0.8);

  //fit->SetParLimits(3,0.5,1);

  TF1 *fit2 = new TF1("fit2",fitFunction,500,700,4);
  fit2->SetParNames("A","E","#sigma","R");

  fit2->SetParameters(150,662,100,0.2);
  fit2->SetParLimits(1,500,700);
